...
public class MotDePasseOublier extends BasePage {
    public static void main(String[] args) {
        ...
        try {
            ...
            emailInput.sendKeys("testteamvivianet1@gmail.com");
            ...
            if (errorMessageText.equalsIgnoreCase("Le lien de réinitialisation du mot de passe a été envoyé à votre adresse e-mail.")) {
...
public class MotDePasseOublier extends BasePage {
    ...
    @Test
    public void testMotDePasseOublier() {
        try {
            // Accéder à la page
            driver.get("https://ppms-sultan-dev.azurewebsites.net/");
            Thread.sleep(1000); // Pause courte pour s'assurer que la page charge
        ...
        System.out.println("MotDePasseOublier : " + testResult);
    }
...
public class MotDePasseOublier extends BasePage {
    ...
    public void setUp() {
        new BasePage();
        wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        System.out.println("[SETUP] Initialisation du test MotDePasseOublier.");
    }

    @Test
    @Description("Vérifie que le lien de réinitialisation du mot de passe fonctionne correctement.")
    @Severity(SeverityLevel.NORMAL)
    public void testMotDePasseOublier() {
        System.out.println("[INFO] Début du test MotDePasseOublier.");
        ...
        try {
            ...
            // Étape 1 : Cliquer sur le bouton "Se connecter"
            By loginButtonLocator = By.xpath("/html/body/div[1]/div/div[1]/div/nav[1]/div[1]/div[3]/div[2]/div/button");
            WebElement loginButton = wait.until(ExpectedConditions.elementToBeClickable(loginButtonLocator));
            loginButton.click();

            // Étape 2 : Cliquer sur le lien "Mot de passe oublié"
            System.out.println("[INFO] Cliquer sur le lien 'Mot de passe oublié'.");
            By motDePasseLienLocator = By.xpath("/html/body/div[3]/div[3]/div/div/p");
            WebElement motDePasseLien = wait.until(ExpectedConditions.elementToBeClickable(motDePasseLienLocator));
            motDePasseLien.click();

            // Étape 3 : Entrer l'email
            System.out.println("[INFO] Entrer l'email dans le champ de réinitialisation.");
            By emailInputLocator = By.xpath("/html/body/div[3]/div[3]/div/div/div/div/div/input");
            WebElement emailInput = wait.until(ExpectedConditions.visibilityOfElementLocated(emailInputLocator));
            emailInput.sendKeys("testteamvivianet1@gmail.com");

            // Étape 4 : Cliquer sur le bouton "Envoyer"
            System.out.println("[INFO] Cliquer sur le bouton 'Envoyer'.");
            By sendButtonLocator = By.xpath("/html/body/div[3]/div[3]/div/div/button");
            WebElement sendButton = wait.until(ExpectedConditions.elementToBeClickable(sendButtonLocator));
            sendButton.click();

            // Étape 5 : Vérifier le message de confirmation ou d'erreur
            System.out.println("[INFO] Vérification du message de confirmation.");
            ...
            //if (errorMessageText.equalsIgnoreCase("test")) {
            if (errorMessageText.equalsIgnoreCase("Le lien de réinitialisation du mot de passe a été envoyé à votre adresse e-mail.")) {
                System.out.println("[SUCCESS] Le message de réinitialisation est correct.");
                testResult = true;
            } else {
                System.out.println("[ERROR] Message incorrect : " + errorMessageText);
                testResult = false;
            }
            ...
            System.out.println("[RESULT] Test MotDePasseOublier : " + (testResult ? "Succès" : "Échec"));
    ...
    public void tearDown() {
        super.tearDown();
        System.out.println("[TEARDOWN] Fin du test MotDePasseOublier.");
    }
}
